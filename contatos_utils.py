...
def contatos_para_pickle(contatos, caminho):
    with open(caminho, mode='wb') as arquivo: # Abrir modo binario de escrita
        pickle.dump(contatos,arquivo) # Garante a segurança do arquivo q esta sendo aberto / lido.
...
def json_para_contatos(caminho):
    ...
    with open(caminho,mode='r') as arquivo:
        ...
        for contato in contatos_json:
            ...
            c = Contato(**contato) # Desempacotar contato
            contatos.append(c)
    return contatos
